...
bool IPP_csv(version v,IPP_CHANNEL c){
    ...
    if(c == RED) {
        file.open(DEFAULT_PATH"MAT_R.csv");
        file << input_img_array[2];
    }
    if(c == GREEN) {
        file.open(DEFAULT_PATH"/MAT_G.csv");
        file << input_img_array[1];
    }
    if(c == BLUE) {
        file.open(DEFAULT_PATH"/MAT_B.csv");
...
bool IPP_hist(version v){
    string input_path = DEFAULT_PATH + to_string(v) + ".jpg";
    string output_path = DEFAULT_PATH"hist.jpg";
    Mat input_img = imread(input_path);
    if(input_img.empty() == true)
        return false;
    Mat output_img;
...
{
    string input_path = DEFAULT_PATH + to_string(v) + ".jpg";
    string output_path = DEFAULT_PATH + (string)"lightchange.jpg";
...
{
    string input_path = DEFAULT_PATH + to_string(v) + ".jpg";
    string output_path = DEFAULT_PATH + (string)"contrastchange.jpg";
